...
proc updateBindingImpl(bindable: var Bindable[bool], boundValues: BoundValueResolver): bool =
  let justCheckPresent = bindable.bindingPattern[0] == '?'

  let bound = if justCheckPresent:
    boundValues.resolve(bindable.bindingPattern.substr(1))
  else:
    boundValues.resolve(bindable.bindingPattern)

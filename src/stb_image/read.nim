...
type
   STBIException* = object of IOError
...
## "Default").
proc load*(filename: string; x, y, channels_in_file: var int; desired_channels: int): ptr[byte] =
   var
      width: cint
      height: cint
      components: cint
   ...
   # Copy pixel data
   let len = x * y * actualChannels
   var pixelData: ptr[byte] = createSharedU(byte, len)
   copyMem(pixelData, data, len)

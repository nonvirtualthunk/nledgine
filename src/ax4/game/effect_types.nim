...
proc readFromConfig*(cv: ConfigValue, d: var DamageExpression) =
  if cv.isStr:
    ...
    d.fixed = cv.asInt
    d.damageType = † DamageTypes.WeaponDamageType
    d.fraction = 1.0f
  else:
...
proc readFromConfig*(cv: ConfigValue, ge: var GameEffect) {.gcsafe.} =
  if cv.isStr:
    ...
    of "derivedAttack":
      ge = GameEffect(kind: GameEffectKind.DerivedAttack, attackSelector: readInto(cv["attackSelector"], AttackSelector), attackModifier: readInto(cv["attackModifier"], AttackModifier))

...
proc asRichText*(de: DamageExpression): RichText =
  ...
  when PrintDamageExpressionsInDicePlusFixedStyle:
    ...
    if de.minDamage == de.maxDamage:
      result = richText(&"{de.minDamage}")
    else:
...
proc asRichText*(view: WorldView, character: Entity, effect: GameEffect, subjectSelector: Option[Selector], targetSelector: Option[Selector]): RichText =
  ...
  of GameEffectKind.SimpleAttack:
    let attack = if character.isSentinel:
      some(effect.attack)
    else:
      effectiveAttack(view, character, effect)

    if attack.isSome:
      asRichText(view, character, attack.get)
    else:
      richText("could not resolve simple attack?")

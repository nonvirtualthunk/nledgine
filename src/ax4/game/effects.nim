...
proc effectiveAttack*(view: WorldView, character: Entity, effect: GameEffect): Option[Attack] =
  withView(view):
    if effect.kind == GameEffectKind.DerivedAttack:
      ...
      if not character.isSentinel:
        for item in equippedItems(view, character):
          if item[Item].attackModifiers.isSome:
            attack.applyModifiers(item[Item].attackModifiers.get)
          if item.hasData(Weapon):
            attack.applyModifiers(item[Weapon].weaponModifiers)
...
proc expandCosts*(view: WorldView, character: Entity, effect: GameEffect): seq[SelectableEffects] =
  case effect.kind:
  of GameEffectKind.DerivedAttack, GameEffectKind.SimpleAttack:
    # Derived attacks that do not have a weapon can't expand costs in a meaningful way
    if effect.kind == GameEffectKind.DerivedAttack and character.isSentinel:
      return @[]
...
proc requiresSelection*(s: EffectGroup, view: WorldView, character: Entity): bool =
  for eff in s.effects:
    if selectionsForEffect(view, character, eff).len != 0:
      return true
  false
...
proc resolveEffect*(world: World, character: Entity, effectPlay: EffectPlay): bool =
  ...
  withWorld(world):
    for effect in effectPlay.effects:
      case effect.kind:
      of GameEffectKind.DerivedAttack:
...
proc toEffectPlayGroup*(view: WorldView, character: Entity, source: Entity, effectGroup: EffectGroup): EffectPlayGroup =
  for effects in effectGroup.effects:
    for effect in effects:
      if effect.kind == GameEffectKind.DerivedAttack or effect.kind == GameEffectKind.SimpleAttack:

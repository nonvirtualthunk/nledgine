...
proc readFromConfig(cv: ConfigValue, v: var Card) =
  ...
  var costs: seq[GameEffect]
  if cv["costs"].nonEmpty:
    cv["costs"].readInto(costs)
  if cv["ap"].nonEmpty:
    costs.add(GameEffect(kind: GameEffectKind.ChangeResource, resource: † ResourcePools.ActionPoints, resourceModifier: modifiers.reduce(cv["ap"].asInt)))
  if cv["stamina"].nonEmpty:
    costs.add(GameEffect(kind: GameEffectKind.ChangeResource, resource: † ResourcePools.StaminaPoints, resourceModifier: modifiers.reduce(cv["stamina"].asInt)))
...
proc createCard*(arch: Taxon, world: World): Entity =
  let cardLib = library(CardArchetype)
  let cardArch = cardLib[arch]
  createCard(cardArch, world)

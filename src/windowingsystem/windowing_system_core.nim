...
iterator dependentOn(p : WidgetDimension, axis : Axis, widget : Widget, parent : Widget, children : seq[Widget]) : Dependency =
    case p.kind:
        of WidgetDimensionKind.Fixed: 
            discard
        of WidgetDimensionKind.Relative,WidgetDimensionKind.Proportional,WidgetDimensionKind.ExpandToParent: 
            yield (widget: parent, kind : DependencyKind.Dimensions, axis : axis)
    ...
    if w.parent.isSome:
        ...
        of WidgetDimensionKind.Intrinsic: 
            ...
            if intrinsicSize.isSome:
                ...
                warn "intrinsic sized widget with no intrinsic qualities"
                10
        of WidgetDimensionKind.WrapContent: 
            var min : int = 0
            var max : int = 0
...
iterator nineWayImageQuads(nwi : NineWayImage, dim : Vec2i, pixelScale : int) : WQuad =
    ...
    if nwi.drawCenter:
        let centerOffset = imgMetrics.borderWidth * nwi.pixelScale * pixelScale
...
proc attachData*[T](widget : Widget, valueIn : T) =
    ...
    widget.windowingSystem.display.attachData(widget.entity, T, value)
        ...
        if cv.isNumber:
            ...
            matcher(str):
                extractMatches(orientedConstantPattern, distStr, relativeToStr):
                    ...
                    else: warn "failed to parse widget oriented constant position : ", str
                extractMatches(orientedProportionPattern, propStr, relativeToStr):
                    ...
                    else: warn "failed to parse widget oriented proportional position : ", str
                extractMatches(rightLeftPattern, distStr, dirStr, targetStr):
                    ...
                    if target.isSome:
                        ...
                        warn "failed to parse widget right/left relative position : ", str

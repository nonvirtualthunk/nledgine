...
type
  TextDisplay* = object
    ...
    conditionalText*: seq[ConditionalText]
    fontSize*: int
...
defineSimpleReadFromConfig(ConditionalText)
...
proc computeLayout*(widget: Widget, inbounds: Recti = rect(vec2i(0, 0), vec2i(10000000, 1000000))): TextLayout =
  ...
  result = layout(effectiveText, TD.fontSize * widget.windowingSystem.pixelScale, bounds, widget.windowingSystem.pixelScale, renderSettings)
...
proc updateAllBindings(td: var TextDisplay, resolver : var BoundValueResolver) : bool =
  result = updateBindings(td, resolver)
  for ct in td.conditionalText.mitems:

...
proc globalIlluminationSettings*(world: LiveWorld, activeRegion : Entity) : GlobalIlluminationSettings =
  ...
  result.ambientLight = case timeOfDay:
    of DayNight.Day: sigmoided * 0.85 + 0.15d
...
proc render(g: WorldGraphicsComponent, world: LiveWorld, display: DisplayWorld) =
  ...
  withWorld(world):
    ...
    let reg: ref Region = activeRegion[Region]
    ...
    for x in max(povEntPos.x - visionRange - 30, -RegionHalfSize + 1) .. min(povEntPos.x + visionRange + 30, RegionHalfSize - 1):
      for y in countdown(min(povEntPos.y + visionRange + 30, RegionHalfSize - 1), max(povEntPos.y - visionRange - 30, -RegionHalfSize + 1)):
        let dx = x - povEntPos.x
        let dy = y - povEntPos.y
        let d2 = dx*dx + dy*dy
        # if d2.float > visionRange2.float32 + 30.0f: continue
        ...
        let opaque = opacity(reg, x, y, MainLayer) > 200

        let baselineGlobalIllumination = if not opaque:
          gis.globalIlluminationAt(x,y,0,0)
        else:
          gis.globalIlluminationAt(x + normal.x, y + normal.y, 0, 0)

        if not opaque:
          localIlluminationAt(lis, x, y, 0, 0, localLightStrength, localLightColor)
        else:
          localIlluminationAt(lis, x + normal.x, y + normal.y, 0, 0, localLightStrength, localLightColor)
        let baselineLocalIllumination = localLightStrength

        let revealed = t.revealed
        ...
        for sy in 0 ..< VisionResolution:
          for sx in 0 ..< VisionResolution:
            ...
            let v = effectiveVision(vision, baselineGlobalIllumination, baselineLocalIllumination, visionRange, dx, dy, sx, sy)

            if v > 0.0 or revealed:
              ...
              if v > 0.0:
                let effV = max(max(gi, localLightStrength) * v, 0.15)
                maxEffV = max(maxEffV, effV)
                qb.color = mix(gis.ambientLightColor, localLightColor, gi, localLightStrength) * effV
                qb.color.a = 1.0
              else:
                qb.color = rgba(0.1,0.1,0.1,1.0)
              qb.drawTo(g.canvas)
              ...
              if t.wallLayers.nonEmpty:
                ...
                if v > 0.0:
                  qb.color = mix(gis.ambientLightColor, localLightColor, normGI, localLightStrength) * max(max(normGI, localLightStrength) * v, 0.15)
                  qb.color.a = 1.0
                else:
                  qb.color = rgba(0.1,0.1,0.1,1.0)
                qb.drawTo(g.canvas)
        ...
        if pi.isSome and pi.get mod 7 == 0 and (maxVision > 0.01 or revealed):
          ...
          if tilesetTile.decor.len > 0 and t.wallLayers.isEmpty:
            ...
            if maxVision > 0.0:
              qb.color = premixedColor * max(max(maxLocalLight, maxGlobalLight) * maxVision, 0.15)
              qb.color.a = 1.0
            else:
              qb.color = rgba(0.1,0.1,0.1,1.0)
        ...
        if maxVision > 0.01 or revealed:
          var offset = 0
          for ent in t.entities:
            let phys = ent.data(Physical)
            # things that move themselves don't get shown just because they have been revealed
            # i.e. a tree still shows up on the map once revealed even if not in sight, a spider does not
            if phys.dynamic and maxVision <= 0.01:
              continue


            var img = phys.images[0].asImage
            if ent.hasData(Player):
              # img = image("survival/graphics/creatures/player/adventurer-idle-01.png")
              img = case phys.facing:
                of Direction.Left: image("survival/graphics/creatures/player/left_bardo.png")
                of Direction.Up: image("survival/graphics/creatures/player/up_bardo.png")
                of Direction.Right: image("survival/graphics/creatures/player/right_bardo.png")
                of Direction.Down: image("survival/graphics/creatures/player/down_bardo.png")
                of Direction.Center: image("survival/graphics/creatures/player/down_bardo.png")
            var gi = 0.0
            if phys.blocksLight:
            ...
            if phys.capsuled and not phys.drawFullWhenCapsuled:
              ...
              let yOffset = if img.height > TileSize.int: 0.0 else: (TileSize - img.height.float) * 0.5
              qb.position = vec3f(x.float * TileSize + (TileSize - img.width.float32) * 0.5f32, y.float * TileSize + yOffset, 10.0f)


            qb.texture = img

            if maxVision > 0.0:
              qb.color = premixedColor * max(gi * maxVision, 0.15)
              qb.color.a = 1.0
            else:
              qb.color = rgba(0.1,0.1,0.1,1.0)
            ...
            # Disabling the explicit facing arrow while we have a sprite that actually does facing

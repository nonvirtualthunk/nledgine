...
template bufferData*(target:GLenum,size:int, data:pointer, usage:GLenum)  =
    glBufferData(target,size.GLsizeiptr,data,usage)
...
template genTexture*() : TextureIdType =
    var tex : GLuint
    glGenTextures(1.GLsizei,addr tex)
    tex.TextureIdType

template genTextures*(count:int32) : seq[TextureIdType] =
    let textures = newSeq[TextureIdType](count)
    glGenTextures(count.GLsizei,cast[ptr GLuint](textures[0].unsafeAddr))
    textures

template genBindTexture*(target:GLenum) : TextureIdType = 
    var tex : GLuint
    glGenTextures(1.GLsizei,addr tex)
    glBindTexture(target, tex)
    tex.TextureIdType

template bindTexture*(target:GLenum, texture:TextureIdType) =
...
#template GenerateTextureMipmap*(texture:TextureIdType) =
...
template shaderSource*(shader:ShaderId, src: string) =
    ...
    glShaderSource(shader.GLuint, 1.GLsizei, cast[ptr cstring](cstr), nil)
...
template setMat4*(program:ShaderProgramId, name: string, value: var Mat4f ) =
    glUniformMatrix4fv(getUniformLocation(program,name).GLint,1,false.GLboolean,value.caddr)
